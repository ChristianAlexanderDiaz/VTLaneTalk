...
import {
  ...
  addDoc,
  getDoc,
} from "firebase/firestore"; // Import Firestore services
...
/**
 * Ensures that all absent bowlers (not in the nullList) have their scores arrays padded with null values
 * up to the current maxScoreArrayLength.
 *
 * @param {Array} bowlers - The complete list of bowlers.
 * @param {Array} nullList - The list of all bowlers who have been processed (active at least once).
 * @param {number} maxScoreArrayLength - The current maxScoreArrayLength.
 */
async function handleAbsentBowlers(bowlers, nullList, maxScoreArrayLength) {
  for (const bowler of bowlers) {
    // Skip bowlers that are already in the nullList (processed)
    if (!nullList.includes(bowler.id)) {
      ...
      // Ensure their scores array matches the current maxScoreArrayLength
      while (databaseScoresArray.length < maxScoreArrayLength) {
        databaseScoresArray.push(null); // Add nulls to match the length
      }

      // Update Firestore with the padded scores array
      await updateDoc(bowlerRef, {
        scores: databaseScoresArray,
      });

      console.log(
        `Updated absent bowler ${bowler.name} with null values up to length ${maxScoreArrayLength}`
      );
    }
  }
}
...
function calculateAverage(scoresArray) {
  ...
  const validScores = scoresArray.filter(
...
async function promptForMaxScoreArrayLength(currentLength) {
  ...
  return new Promise((resolve) => {
    rl.question(
      `maxScoreArrayLength found: ${currentLength}. Is this correct? (yes/no) `,
      async (answer) => {
        if (answer.toLowerCase() === "yes" || answer.toLowerCase() === "y") {
          ...
          rl.question(
            "Please enter the correct maxScoreArrayLength: ",
            async (newLength) => {
                ...
                try {
                  ...
                  if (!querySnapshot.empty) {
                    ...
                    await updateDoc(docRef, {
                      maxScoreArrayLength: parsedLength,
                    });
                    console.log(
                      `maxScoreArrayLength successfully updated to ${parsedLength} in the database.`
                    );
                    rl.close();
                    resolve(parsedLength);
                  } else {
                    console.log(
                      "No document found in the 'variables' collection to update."
                    );
                    rl.close();
                    resolve(currentLength);
                  }
                  ...
                  console.error(
                    "Error updating maxScoreArrayLength in the database: ",
                    error
                  );
                  rl.close();
...
async function getMaxScoreArrayLength() {
  ...
  if (!querySnapshot.empty) {
    ...
    if (
      data.maxScoreArrayLength &&
      typeof data.maxScoreArrayLength === "number"
    ) {
...
async function scrapeData() {
  // Set up Puppeteer and navigate to the target URL
    ...
    try {
      ...
      data = await page.evaluate(() => {
        ...
        return players.map((player) => {
          const nameText = player.querySelector(
            ".nameAndDate > .name"
          ).innerText;
      ...
      if (attemptCount >= maxAttempts) {
        console.error(
          "Maximum number of attempts reached. Unable to load the page."
        );
        throw error; // Re-throw the error if max attempts are reached
      }

      console.log(
        `Retrying... (${maxAttempts - attemptCount} attempts left)\n`
      );
    }
  }
...
async function addNewBowler(name, nicknames = [], maxScoreArrayLength) {
  ...
  console.log(
    `Added new bowler: ${name} with nicknames: ${nicknames.join(", ")}`
  );
}
...
async function promptForNewBowlers(maxScoreArrayLength) {
  ...
  while (addMore) {
    await new Promise((resolve) => {
      rl.question(
        "Enter the name of the new bowler (or press Enter to stop adding): ",
        async (name) => {
          if (name) {
            rl.question(
              "Do they have any nicknames? (Separate with commas if multiple): ",
              async (nicknamesInput) => {
                const nicknames = nicknamesInput
                  ? nicknamesInput.split(",").map((n) => n.trim())
                  : [];
...
function findBowlerByNameOrNickname(playerName, bowlers) {
  ...
  return bowlers.find(
    (bowler) =>
      ...
      (bowler.nicknames &&
        bowler.nicknames.map(normalizeName).includes(normalizedPlayerName))
  );
}
...
async function main() {
  ...
  let permanentActiveList = []; // Track bowlers that have been in the activeList at least once
  ...
  for (const bowler of bowlers) {
    ...
    if (
      databaseScoresArray[firstGameIndex] &&
      !databaseScoresArray[secondGameIndex]
    ) {
      console.log(
        `${bowler.name} has a first score but not a second score, adding them to the active list.`
      );
  ...
  while (true) {
    // Keep looping until we manually break
    ...
    for (const player of filteredData) {
      ...
      if (bowler) {
        // If the bowler exists in the database, begin the process
        ...
        if (!activeList.includes(bowler.id)) {
          if (
            ...
            console.log(
              `${bowler.name}'s first score updated to ${player.scoresArray[0]}`
            );

            activeList.push(bowler.id); // Add bowler to the active list

            // Add to permanentActiveList if not already in it
            if (!permanentActiveList.includes(bowler.id)) {
              permanentActiveList.push(bowler.id);
            }

            // Add to nullList if not already in it
        ...
        // Case 2: Bowler is already in the active list, update the second score and remove them from the active list
          ...
          if (
            ...
            await updateBowlerAverage(bowler.id, databaseScoresArray); // Update the bowler's average
            console.log(
              `${bowler.name}'s second score updated to ${player.scoresArray[1]}`
            );
    ...
    if (activeList.length === 0 && hasProcessedAtLeastOneActiveBowler) {
      console.log(
        "All bowlers have completed both scores, stopping the script."
      );
      await handleAbsentBowlers(
        bowlers,
        permanentActiveList,
        maxScoreArrayLength
      ); // Handle absent bow
